...
**Scene order in the `App` body decides what launch opens — `Settings` must never be first.** SwiftUI treats the first scene as the app's primary one: it is what a cold launch shows, what a Dock click reopens, and what "open untitled" resolves to. `Settings` was declared before the `WindowGroup`, so an app quit while the canvas window happened to be closed relaunched showing **only the settings window**, and clicking the Dock icon opened a _second_ settings window rather than the canvas — leaving File ▸ New Window as the only way back into the app. It is sticky and it survives a reinstall, because nothing is persisted for it to recover from; the app is simply reopening its primary scene. Reproduce in three steps: launch, ⌘, then ⌘W the canvas, ⌘Q, relaunch. Verified on macOS 26.5. `WindowGroup(id: SynesthiaApp.mainWindowID)` is now first and `Settings` last, and Window ▸ Synesthia (⌘0) exists as a named way back if a user ever ends up windowless again.

**Nothing plays until the welcome sheet is answered.** `AppState.onAppear` returns early while `showsWelcome` is true, and `completeWelcome()` starts the demo only when `firstRunDemoPending` is still set — i.e. only when the user clicked Continue without choosing a source. So the App Review path ("grants nothing, must still see it work") is Continue, not launch; `docs/app-store-metadata.md` says so and must stay in step. `WelcomeView.SourceRow` therefore calls `selectSource` **before** `completeWelcome`: the reverse order starts the demo and then immediately tears it down to switch sources.
...
**Squash-merging a release branch orphans its tag, and nothing tells you.** `bump-version.sh` refuses to run on `main` by design, so every release is cut on a branch and has to be merged back. Squashing rewrites the tagged commit; `--delete-branch` then removes the last ref that could reach the original, and `v<version>` points into nothing. `git describe` on `main` stops seeing the release, `resolve_base` drafts the next release's notes against the wrong range, and the DMG in R2 corresponds to a commit in no branch's history. `v1.2` was in that state until it was re-tagged onto `944b50a` — the squash commit on `main` whose tree is byte-identical to the orphaned one, which is the only reason the repair was safe. That cost a force-push of a published tag; do it once, not as a habit. **Release branches merge with `--merge`, never `--squash`** (ordinary work still squashes; it has no tag to orphan). Two guards make this structural: `publish-release.sh` refuses to publish a version whose tag is not an ancestor of `origin/main` (`--allow-unmerged` overrides), and `release-integrity.yml` files a `needs:human` issue when a tag, `VERSION`, or the published feed drift apart.
