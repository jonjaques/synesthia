...
Consent, and Control") — the system that shows "*App X would like to…*"
prompts and remembers the answer in System Settings › Privacy & Security.
Denials usually surface as *errors or empty results, not crashes*, which is
...
deliberately *outside* the `Synesthia/` source folder so Xcode's synced
...
Sandbox violations are the classic cause of *silent* failures (e.g. reading
...
- **AppleScript constants don't coerce to text**: `player state as text`
  *throws*; the script maps the constant to a string with `is playing`
...
window *title* is still set (`navigationTitle`) so Mission Control and the
...
- **Main-actor by default.** `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`
  ...
  `@MainActor` annotations are redundant — instead, *background* work must
